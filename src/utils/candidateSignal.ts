...
const SECTOR_RULES: Array<{ name: string; keywords: string[] }> = [
  { name: "人工智能", keywords: ["工业富联", "寒武", "浪潮", "中科曙光", "海光", "昆仑", "拓维"] },
  { name: "半导体", keywords: ["长电", "通富", "华天", "北方华创", "中微", "兆易", "韦尔", "士兰"] },
  { name: "芯片", keywords: ["芯原", "晶方", "华虹", "闻泰", "紫光"] },
  { name: "通信", keywords: ["中兴", "通宇", "光迅", "新易盛", "中际", "天孚", "亨通", "烽火"] },
  ...
  { name: "有色金属", keywords: ["黄金", "紫金", "山东黄金", "中金黄金", "洛阳钼业"] },
  { name: "机器人", keywords: ["机器人", "埃斯顿", "汇川", "鸣志", "拓斯达"] },
];
...
export function buildStockCandidate(
  ...
  if (changePercent >= 7 || changePercent <= -6) return null;
  ...
  const volumeBreak60 =
    ...
    changePercent > -1 &&
    changePercent < 6.5;
  const macdConfirm = !!daily?.macdGoldZero && changePercent > -1.5 && changePercent < 6.5;
  const nearMa60 =
  ...
  const hasPrimarySignal = volumeBreak60 || macdConfirm || pullbackHold;
  ...
  const risk = daily
    ? `不追高；若跌回60日线下方或放量跌破${daily.ma60.toFixed(2)}附近，放弃/机械止损。`
    : "缺少日线指标，只作为盘中观察；若板块转弱或放量下杀，放弃。";
